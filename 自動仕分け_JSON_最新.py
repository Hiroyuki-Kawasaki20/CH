...
class App(tb.Window):
    ...
    def delete_process_rules_selected(self):
        ...
        self.recompute_process_assignment()
        self.update_kb_views()
    # 置換：工程割当（全サイズの山を対象に、納入先ベースで行う）
    def recompute_process_assignment(self):
        """
        工程割当は納入先ベース。対象は全サイズ種類の“山”。
        - 種類1混載山 + その他サイズの基本グループを build_all_mountain_details で統合
        - 各行の工程は process_map（納入先→工程）で付与
        - 山工程は compute_mountain_process（優先度/多数決）で決定。武部のみの山は工程3を強制（任意）
        """
        try:
            ...
            strategy = "priority"
            self.mountain_proc = compute_mountain_process(self.proc_details, strategy=strategy, force_takebe_to_3=False)
