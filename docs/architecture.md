...
- 締切は「入車時間 − `PICKUP_DEADLINE_BUFFER_SECS`」を基本に扱います（値は `docs/仕分け・割り振りルール.md` 付録A参照。旧記載の「10分」は誤り、実際は20分）。
...
- 工程別引継ぎ時刻: 同一直での連続実行時に前回の各工程終了時刻を保持
  - `selected_shift` の値（1直 / 2直）と同じ直で再実行した場合のみ利用
  - 直が切り替わる、または「引継クリア」で自動/手動リセット
  - アプリ再起動で自動クリア
