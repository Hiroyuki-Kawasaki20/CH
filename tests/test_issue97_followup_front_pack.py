...
def test_issue97_followup_hino04_is_front_packed_without_hino_interleave():
    ...
    assert all(start is not None and start >= floor for start in starts.values())
        ...
        # 実測ベース（19:16/19:26/19:34）に余裕を持たせた上限へ改訂。
        # 2026-09-24 C-6（締切 入車20分前→10分前）: 締切が緩んで山5がリリーフ→メインへ入り、
        # 休憩(18:45)前の枠が埋まって山19・山9が休憩明けへ回ったため、日野04は 19:31/19:41/19:51 へ後退。
        # 日野04先頭の直前の空きは照合時間3分のみで、前詰めは維持されている。実測に余裕を持たせた上限へ改訂。
    assert min(starts.values()) <= pa._time_to_seconds("19:35")
    assert max(starts.values()) <= pa._time_to_seconds("19:55")
    ...
    assert sum(1 for lane in lanes if lane == main_label) >= 17