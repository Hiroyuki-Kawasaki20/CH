...
def test_issue97_followup_hino04_is_front_packed_without_hino_interleave():
    ...
    # (d) 日野04便3山の開始上限（前詰めの見張り）。
    #   2026-08-25 河崎裁定: 当時の短休憩は引取不可30分(18:45-19:15)。当初期待値
    #   18:46/18:53/19:01 は休憩帯内で達成不能のため撤回し、実測 19:16/19:26/19:34 に
    #   余裕を持たせた上限 19:20/19:40 とした。
    #   2026-09-24 C-6(現場合意): 短休憩を引取不可20分(18:45-19:05)に短縮。空いた19:06以降の
    #   窓には締切の早い山17(20:25。リリーフ→メインへ復帰)・山9(19:29)・山19(20:25)が先に入り、
    #   日野04(締切21:08)の実測は 19:34/19:47/19:54 へ後ろ倒しになった。これはリリーフ削減
    #   ((f)で見張る)との引き換えで、締切(b)と床(c)は守られている。上限は旧方針と同じく
    #   実測+約5分の余裕で 19:40/20:00 とする。
    assert min(starts.values()) <= pa._time_to_seconds("19:40")
    assert max(starts.values()) <= pa._time_to_seconds("20:00")

    # (e) 2026-09-22 裁定(#166): 「全山の終了が19:45以内」という上限固定は撤回する。
    #   あふれ(3人目)/リリーフの削減は定義上ほぼ必ず全体終了を後ろへ伸ばすため、
    #   旧上限は人員削減と両立しない当時のスナップショットだった（実測20:34）。
    #   守るべき要件は (b) の締切内であり、終了時刻そのものではない。
    #   本テストの主題である日野04便3山の終了だけを見張る。
    #   2026-09-24 C-6: 実測19:59で旧上限20:00との余裕が1分しかないため、(d)と同じく
    #   実測+約5分の 20:05 へ改訂。
    assert max(
        pa._time_to_seconds(str(by_yama[yama]["実終了時間"])) for yama in hino04
    ) <= pa._time_to_seconds("20:05")

    # (f) 回帰ガード(#166): あふれ0山を維持し、メインの山数が後退しないこと。
    #   2026-09-24 C-6: 短休憩短縮で山17がリリーフからメインへ戻りメイン19山になったため、
    #   この改善を守るよう下限を 17 → 19 へ引き上げる。
    lanes = [str(row["山工程"]) for row in by_yama.values()]
    assert not [lane for lane in lanes if "あふれ" in lane]
    main_label = getattr(pa, "PROC_MAIN", "1工程")
    assert sum(1 for lane in lanes if lane == main_label) >= 19